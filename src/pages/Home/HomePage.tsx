...
export function HomePage() {
  ...
  const consultationsPath = localizedPath("consultations", language);
  const homePath = localizedPath("home", language);

  const principles = [
    {
      icon: ShieldCheck,
      title: t("home.principles.free.title"),
      description: t("home.principles.free.body"),
    },
    {
      icon: Braces,
      title: t("home.principles.documented.title"),
      description: t("home.principles.documented.body"),
    },
    {
      icon: Zap,
      title: t("home.principles.favorites.title"),
      description: t("home.principles.favorites.body"),
    },
  ];
        ...
        <Container className="relative grid gap-12 py-16 md:py-20 lg:grid-cols-[1.02fr_0.98fr] lg:items-center lg:py-24">
          <div className="lg:py-6">
            <HeroBadge icon={Sparkles} className="mb-5">
              {t("home.hero.badge", { count: tools.length })}
            </HeroBadge>
            ...
            <p className="mt-4 max-w-2xl text-lg leading-8 text-ink-500">{t("home.hero.description")}</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Button href={toolsPath}>
                {t("home.hero.ctaPrimary")}
                <ArrowRight className="ml-2" size={17} />
              </Button>
              <Button href={consultationsPath} variant="secondary">
          ...
          <div className="grid gap-4 lg:pl-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl border border-surface-200/75 bg-surface-50/90 p-4 shadow-panel ring-1 ring-surface-50/80 backdrop-blur">
                ...
                <p className="mt-1 text-sm text-ink-500">{t("home.stats.toolsLabel")}</p>
              </div>
              <div className="rounded-xl border border-surface-200/75 bg-surface-50/90 p-4 shadow-panel ring-1 ring-surface-50/80 backdrop-blur">
                <span className="text-3xl font-semibold text-ink-900">{categories.length}</span>
                <p className="mt-1 text-sm text-ink-500">{t("home.stats.categoriesLabel")}</p>
              </div>
              <div className="rounded-xl border border-surface-200/75 bg-surface-50/90 p-4 shadow-panel ring-1 ring-surface-50/80 backdrop-blur">
                <span className="text-3xl font-semibold text-ink-900">{integrableToolsCount}</span>
                <p className="mt-1 text-sm text-ink-500">{t("home.stats.integrableLabel")}</p>
              </div>
              <div className="rounded-xl border border-ink-700/25 bg-gradient-to-br from-ink-900 to-ink-700 p-4 text-surface-50 shadow-panel ring-1 ring-surface-50/20">
                ...
                <p className="mt-1 text-sm text-surface-200">{t("home.stats.uploadedLabel")}</p>
              </div>
            </div>
            <div className="rounded-xl border border-accent-cyan/20 bg-surface-50/90 p-4 shadow-panel ring-1 ring-surface-50/80 backdrop-blur">
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-ink-900">{t("home.localPanel.title")}</p>
                  <p className="mt-1.5 text-sm leading-6 text-ink-500">{t("home.localPanel.body")}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="border-t border-surface-50/80 bg-gradient-to-b from-surface-100/40 to-transparent">
        <Container className="py-14">
          <div className="w-full">
            ...
            <h2 className="text-2xl font-semibold text-ink-900 md:text-3xl">{t("home.why.title")}</h2>
            <p className="mt-2 w-full text-base leading-7 text-ink-500">{t("home.why.description")}</p>
          </div>
          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-3">
            {principles.map((principle) => {
              ...
              return (
                <article
                  key={principle.title}
                  className="group rounded-xl border border-surface-200/85 bg-gradient-to-br from-surface-50/95 to-surface-100/60 p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-accent-cyan/40 hover:from-surface-50 hover:to-surface-50/90 hover:shadow-panel motion-reduce:hover:translate-y-0"
                >
