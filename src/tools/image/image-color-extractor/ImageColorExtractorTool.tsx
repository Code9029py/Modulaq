...
const copy = {
  es: {
    ...
    estimation: "Los colores son una estimacion basada en muestreo local.",
    outputName: "Nombre de la paleta",
    outputTitle: "Paleta",
    paletteReady: "Paleta lista",
    processing: "Analizando imagen...",
    samplePixels: "Pixeles muestreados",
    sourceIntro: "Extrae una paleta aproximada de colores dominantes desde una imagen.",
    sourceTitle: "Imagen de origen",
    swatch: "Muestra de color",
    totalLocal: "Todo se procesa en tu navegador.",
  },
  en: {
    ...
    estimation: "Colors are an estimate based on local sampling.",
    outputName: "Palette name",
    outputTitle: "Palette",
    paletteReady: "Palette ready",
    processing: "Analyzing image...",
    samplePixels: "Sampled pixels",
    sourceIntro: "Extract an approximate palette of dominant colors from an image.",
    sourceTitle: "Source image",
    swatch: "Color swatch",
    totalLocal: "Everything is processed in your browser.",
  },
} as const;
...
export function ImageColorExtractorTool() {
  ...
  const canAnalyze = Boolean(file && metadata) && status !== "reading" && status !== "processing";
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(320px,0.58fr)]">
      <section className="min-w-0 rounded-2xl border border-surface-200/80 bg-gradient-to-br from-surface-50/95 to-surface-100/50 p-4 shadow-panel ring-1 ring-surface-50/80 backdrop-blur">
        <div className="grid gap-4">
          ...
          <div className="grid gap-3 rounded-xl border border-surface-200/80 bg-surface-50/80 p-3 shadow-sm">
            ...
            <p className="text-xs leading-5 text-ink-500">{labels.estimation}</p>
            <p className="text-xs leading-5 text-ink-500">{labels.totalLocal}</p>
          </div>
      ...
      <section className="min-w-0 rounded-2xl border border-surface-200/80 bg-gradient-to-br from-surface-50/95 to-surface-100/50 p-4 shadow-panel ring-1 ring-surface-50/80 backdrop-blur">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-ink-500">{labels.estimation}</p>
        </div>
        ...
        <div className="mt-5 grid gap-3 rounded-xl border border-surface-200/80 bg-surface-50/80 p-4 shadow-sm">
          {colors.length > 0 ? (
            <div className="grid gap-3">
              <div className="rounded-lg border border-accent-teal/25 bg-accent-teal/10 p-3 text-sm text-ink-700">
                <p className="font-semibold text-ink-900">{labels.paletteReady}</p>
              ...
              {colors.map((color: ExtractedImageColor) => {
                ...
                return (
                  <div key={color.hex} className="grid gap-3 rounded-lg border border-surface-200/80 bg-surface-50/90 p-3 shadow-sm sm:grid-cols-[72px_minmax(0,1fr)]">
                    <div
                    ...
                    <div className="grid min-w-0 gap-2 text-sm">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                          ...
                          <p className="text-xs text-ink-500">{rgb}</p>
                        </div>
            ...
            <div className="grid min-h-56 place-items-center rounded-lg border border-surface-200/80 bg-surface-50/90 p-4 text-center text-sm text-ink-500">
              <span>
          ...
          <label className="grid gap-2 text-sm font-semibold text-ink-700">
            {labels.outputName}
            <input
              className="min-h-11 rounded-lg border border-surface-200/90 bg-surface-50/95 px-3 text-sm font-normal text-ink-900 shadow-sm outline-none transition placeholder:text-ink-500/70 focus:border-accent-cyan focus:bg-surface-50 focus:ring-2 focus:ring-accent-cyan/25"
