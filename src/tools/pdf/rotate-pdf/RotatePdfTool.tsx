...
import {
  ...
  rotatePdfFromFile,
  type PdfRotation,
} from "./rotatePdf.service";
import type { RotatePdfMetadata, RotatePdfStatus } from "./rotatePdf.types";
...
const rotationLabelKeys: Record<PdfRotation, TranslationKey> = {
  90: "tools.rotate-pdf.ui.rotation90",
  180: "tools.rotate-pdf.ui.rotation180",
  270: "tools.rotate-pdf.ui.rotation270",
};
...
export function RotatePdfTool() {
  ...
  const [metadata, setMetadata] = useState<RotatePdfMetadata | null>(null);
  const [rotation, setRotation] = useState<PdfRotation>(90);
  const [status, setStatus] = useState<RotatePdfStatus>("idle");
  ...
  const [lastResult, setLastResult] = useState<{ pageCount: number; rotation: PdfRotation } | null>(null);
  ...
  const canRotate = Boolean(metadata) && !isBusy;
  ...
  const handleRotate = async () => {
    ...
    try {
      const result = await rotatePdfFromFile(file, rotation, outputBaseName, localizedDefaultOutputBaseName);
      downloadPdf(result.bytes, result.fileName);
      setLastResult({ pageCount: result.pageCount, rotation: result.rotation });
      setStatus("success");
    } catch (nextError) {
      setStatus("error");
      setError(resolveToolErrorMessage(nextError, t, "tools.errors.rotatePdfFailed"));
    }
  };
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(300px,0.55fr)]">
      <section className="min-w-0 rounded-2xl border border-surface-200/80 bg-gradient-to-br from-surface-50/95 to-surface-100/50 p-4 shadow-panel ring-1 ring-surface-50/80 backdrop-blur">
        <div className="grid gap-4">
          <div>
            <h3 className="text-sm font-semibold text-ink-900">{t("tools.rotate-pdf.ui.fileSection")}</h3>
            <p className="mt-1 text-sm leading-6 text-ink-500">{t("tools.rotate-pdf.ui.intro")}</p>
          </div>
          ...
          <fieldset className="grid gap-2">
            <legend className="text-sm font-semibold text-ink-700">{t("tools.rotate-pdf.ui.rotationLabel")}</legend>
            <div className="grid grid-cols-3 gap-2">
              {PDF_ROTATIONS.map((option) => {
                const isActive = rotation === option;
                return (
                  <button
                    key={option}
                    type="button"
                    aria-pressed={isActive}
                    disabled={!metadata || isBusy}
                    className={cn(
                      "inline-flex min-h-10 items-center justify-center gap-1.5 rounded-lg border px-2 py-2 text-sm font-semibold shadow-sm transition focus:outline-none focus:ring-2 focus:ring-accent-cyan/25",
                      isActive
                        ? "border-ink-900 bg-ink-900 text-surface-50"
                        : "border-surface-200/90 bg-surface-50/95 text-ink-700 hover:border-accent-cyan/55 hover:text-ink-900",
                      (!metadata || isBusy) && "cursor-not-allowed opacity-50",
                    )}
                    onClick={() => {
                      setRotation(option);
                      resetFeedback();
                  ...
                  >
                    <RotateCw size={14} />
                    {t(rotationLabelKeys[option])}
                  </button>
                );
              })}
            </div>
            <p className="text-xs font-normal text-ink-500">{t("tools.rotate-pdf.ui.rotationHelp")}</p>
          </fieldset>
        </div>
      </section>
      ...
      <section className="min-w-0 rounded-2xl border border-surface-200/80 bg-gradient-to-br from-surface-50/95 to-surface-100/50 p-4 shadow-panel ring-1 ring-surface-50/80 backdrop-blur">
        <div>
          <h3 className="text-sm font-semibold text-ink-900">{t("tools.rotate-pdf.ui.outputTitle")}</h3>
          <p className="mt-1 text-xs leading-5 text-ink-500">{t("tools.rotate-pdf.ui.outputIntro")}</p>
        </div>
        ...
        <div className="mt-5 grid gap-3 rounded-xl border border-surface-200/80 bg-surface-50/80 p-4 shadow-sm">
          ...
          {status === "processing" ? (
            <p className="flex items-center gap-2 rounded-lg border border-surface-200/80 bg-surface-50/90 px-3 py-2 text-sm text-ink-600 shadow-sm">
              ...
              {t("tools.rotate-pdf.ui.processing")}
            </p>
          ) : null}

          {status === "success" && lastResult ? (
            <p className="rounded-md border border-accent-teal/25 bg-accent-teal/10 px-3 py-2 text-sm text-ink-700">
              {t("tools.rotate-pdf.ui.successSummary", { pages: lastResult.pageCount, rotation: lastResult.rotation })}
            </p>
          ) : null}
          ...
          <div className="grid gap-2 pt-1">
            <Button type="button" className="gap-2" onClick={() => void handleRotate()} disabled={!canRotate}>
              {status === "processing" ? <Loader2 className="animate-spin" size={16} /> : <Download size={16} />}
              {t("tools.rotate-pdf.ui.cta")}
            </Button>
          ...
          <p className="mt-2 flex items-start gap-2 text-xs leading-5 text-ink-500">
            ...
            {t("tools.rotate-pdf.ui.localProcessingNote")}
          </p>
        </div>
      </section>
    </div>
