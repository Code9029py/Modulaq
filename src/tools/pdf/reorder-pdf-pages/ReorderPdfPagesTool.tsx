...
export function ReorderPdfPagesTool() {
  ...
  const movePage = (position: number, direction: -1 | 1) => {
    setPageOrder((currentPageOrder) => {
      ...
      if (targetPosition < 0 || targetPosition >= currentPageOrder.length) return currentPageOrder;
      const nextPageOrder = [...currentPageOrder];
      [nextPageOrder[position], nextPageOrder[targetPosition]] = [nextPageOrder[targetPosition], nextPageOrder[position]];
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      setLastOutput(
        t("tools.reorder-pdf-pages.ui.downloadedSummary", {
          name: result.fileName,
          count: result.pageCount,
          label: result.pageCount === 1 ? t("toolUi.pagesSingular") : t("toolUi.pagesPlural"),
        }),
      );
      setStatus("success");
    } catch (nextError) {
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,0.96fr)_minmax(310px,0.52fr)]">
      <section className="min-w-0 rounded-2xl border border-surface-200/80 bg-gradient-to-br from-surface-50/95 to-surface-100/50 p-4 shadow-panel ring-1 ring-surface-50/80 backdrop-blur">
        <div className="grid gap-4">
          <div>
            <h3 className="text-sm font-semibold text-ink-900">{t("tools.reorder-pdf-pages.ui.section")}</h3>
            <p className="mt-1 text-sm leading-6 text-ink-500">{t("tools.reorder-pdf-pages.ui.intro2")}</p>
          </div>
          ...
          {metadata ? (
            <div className="grid gap-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-xs font-semibold uppercase tracking-wide text-ink-500">{t("tools.reorder-pdf-pages.ui.orderLabel")}</p>
                <Button
                  type="button"
                  variant="ghost"
                ...
                >
                  <RotateCcw size={15} />
                  {t("tools.reorder-pdf-pages.ui.restore")}
                </Button>
              </div>
              ...
              <ul className="grid max-h-[26rem] gap-2 overflow-y-auto pr-1">
                {pageOrder.map((pageNumber, position) => (
                  ...
                  >
                    ...
                    <p className="text-sm font-semibold text-ink-900">{t("tools.reorder-pdf-pages.ui.pageLabel", { number: pageNumber })}</p>
            ...
            <div className="rounded-xl border border-surface-200/80 bg-surface-50/80 p-4 shadow-sm">
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-ink-900">{t("tools.reorder-pdf-pages.ui.idleTitle")}</p>
                  <p className="mt-1 text-sm leading-6 text-ink-500">{t("tools.reorder-pdf-pages.ui.idleBody")}</p>
                </div>
              </div>
            </div>
          )}
      ...
      <section className="min-w-0 rounded-2xl border border-surface-200/80 bg-gradient-to-br from-surface-50/95 to-surface-100/50 p-4 shadow-panel ring-1 ring-surface-50/80 backdrop-blur">
        <div>
          <h3 className="text-sm font-semibold text-ink-900">{t("tools.reorder-pdf-pages.ui.outputTitle")}</h3>
          <p className="mt-1 text-xs leading-5 text-ink-500">{t("tools.reorder-pdf-pages.ui.outputIntro2")}</p>
        </div>
        ...
        <div className="mt-5 grid gap-3 rounded-xl border border-surface-200/80 bg-surface-50/80 p-4 shadow-sm">
          <div className="rounded-lg border border-accent-cyan/25 bg-accent-cyan/10 p-4 text-center">
            <p className="text-sm font-semibold text-ink-700">{t("tools.reorder-pdf-pages.ui.pagesDetected")}</p>
            <p className="mt-2 text-5xl font-semibold text-ink-900">{metadata?.pageCount ?? 0}</p>
            <p className="mt-2 text-sm font-semibold text-ink-700">
              {metadata?.pageCount === 1 ? t("toolUi.pagesSingular") : t("toolUi.pagesPlural")}
            </p>
          </div>

          <dl className="grid gap-3 text-sm">
            <div className="rounded-lg border border-surface-200/80 bg-surface-50/90 p-3 shadow-sm">
              <dt className="text-ink-500">{t("tools.reorder-pdf-pages.ui.currentOrder")}</dt>
              <dd className="mt-1 font-semibold text-ink-900">
                {hasOriginalOrder ? t("tools.reorder-pdf-pages.ui.original") : t("tools.reorder-pdf-pages.ui.modified")}
              </dd>
            </div>
            <div className="rounded-lg border border-surface-200/80 bg-surface-50/90 p-3 shadow-sm">
              <dt className="text-ink-500">
                <label htmlFor="reorder-pdf-output-name">{t("toolUi.outputName")}</label>
              </dt>
              <dd className="mt-2">
                <input
          ...
          {status === "reading" || status === "processing" ? (
            <p className="flex items-center gap-2 rounded-lg border border-surface-200/80 bg-surface-50/90 px-3 py-2 text-sm text-ink-600 shadow-sm">
              ...
              {status === "reading" ? t("toolUi.readingPdf") : t("tools.reorder-pdf-pages.ui.generating2")}
            </p>
          ) : null}
          ...
          {!metadata && status === "idle" ? (
            <p className="rounded-lg border border-surface-200/80 bg-surface-50/90 px-3 py-2 text-sm text-ink-600 shadow-sm">
              {t("tools.reorder-pdf-pages.ui.pickFirst")}
            </p>
          ) : null}
          ...
          <div className="grid gap-2 pt-1">
            <Button type="button" className="gap-2" onClick={() => void generatePdf()} disabled={!canDownload}>
              ...
              {t("tools.reorder-pdf-pages.ui.downloadCta")}
            </Button>
